/* TEAM: William Miller and Mike Stevens
   DATE: 4/22/2022
   ABOUT: Models a Player of the game. fields include information about
   scoring, attributes of the player (eg, name and partner), and models
   the play zone (where the played cards are displayed), hand zone (where their
   hand appears on the display) and score zone (where relevant scoring
   information is displayed).
   Important methods:
   playCard() - contains the action listener for the selected
   card to be played and rendered to the play area.
   getBidInput() - creates a JDialog to enable user to enter a bid prior to
   each round. Contains an action listener to respond to user input.
   sortHand() - sorts the hand dealt to each player -- enables greater user
   intuition during play.
   setHandZone() - sets where the cards will be rendered for each player on the
   display
   setPlayZone() - sets where the selected card to be played will be rendered
   on the display.
   setScoreZone() - sets where textual information about the player will be
...
public class Player {
  ...
  // method to play a selected card -- Action Listener set for mouse even
  ...
  public void getBidInput(SpadesPanel sp) {
    ...
    bg.add(rb13);
    bg.add(rb14);
    setBid = new JButton("Select");
    setBid.setBounds(10, 10, 20, 20);

    // ACTION LISTENER for selecting bid and Save & Quit Game
